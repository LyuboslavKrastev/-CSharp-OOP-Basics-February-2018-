...
public class DataMapper
{
    ...
    private static void EnsureFile(string path)
    {
        if (!File.Exists(path))
        {
            File.Create(path).Close(); // close the file stream
        }
    }
    ...
    public static void SaveCategories(List<Category> categories)
    {
        ...
        foreach (var category in categories)
        {
            ...
            var line = string.Format(categoryFormat, category.Id, category.Name, string.Join(",", category.PostIds));
            lines.Add(line);
        }
    ...
    public static void SaveUsers(List<User> users)
    {
        ...
        foreach (var user in users)
        {
            ...
            var line = string.Format(userFormat, user.Id, user.Username, user.Password, string.Join(",", user.PostIds));
            lines.Add(line);
        }
    ...
    public static void SavePosts(List<Post> posts)
    {
        ...
        foreach (var post in posts)
        {
            ...
            var line = string.Format
                (postFormat, post.Id, post.Title, post.Content, post.CategoryId, post.AuthorId, string.Join(",", post.ReplyIds));
            lines.Add(line);
        }
    ...
    public static void SaveReplies(List<Reply> replies)
    {
        ...
        foreach (var reply in replies)
        {
            ...
            var line = string.Format
                (replyFormat, reply.Id, reply.Content, reply.AuthorId, reply.PostId);
            lines.Add(line);
        }
